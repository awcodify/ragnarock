...
class PrometheusClient:
    ...
    def get_node_metrics(self, time_range: int = 3600) -> List[Dict]:
        ...
        metrics = {
            "cpu_usage": 'sum(rate(node_cpu_seconds_total{mode="user"}[5m])) by (instance)',
        ...
        for metric_name, query in metrics.items():
            metric_data = self.client.custom_query_range(
                query=query, start_time=start_time, end_time=end_time, step="5m"
            )
            results.append({"metric": metric_name, "data": metric_data})
